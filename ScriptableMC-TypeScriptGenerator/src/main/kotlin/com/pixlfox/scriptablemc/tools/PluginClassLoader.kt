...
class PluginClassLoader(private val pluginFolder: File) : URLClassLoader(
    pluginFolder
        .listFiles()?.filter {
    ...
    fun buildClassDescriptionList(isClassAllowed: (TypeScriptDefinitionGenerator.ClassDescription) -> Boolean): Array<TypeScriptDefinitionGenerator.ClassDescription> {
        ...
        val jarFiles = pluginFolder.listFiles()?.filter { it.extension.equals("jar", true) }

        if(jarFiles != null) {
            for(jarFile in jarFiles) {
                val zipStream = ZipInputStream(FileInputStream(jarFile.canonicalPath))
